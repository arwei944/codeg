...
pub struct AppState {
    ...
    /// in-process consumers (lifecycle, chat-channel subscribers).
    /// Distinct from `event_broadcaster`, which carries JSON-shaped
    /// `WebEvent`s for transport-bound delivery.
